...
import net.parim.sns.modules.gen.dao.GenDataBaseDictDao;
...
public class GenTableService extends BaseService {
	...
	public GenTable get(Long id) {
		GenTable genTable = genTableDao.findOne(id);
		GenTableColumn genTableColumn = new GenTableColumn();
		genTableColumn.setGenTable(new GenTable(genTable.getId()));
		genTable.setColumnList(genTableColumnDao.findAll(genTableColumn));
		return genTable;
	}
	
	public Page<GenTable> find(GenTable genTable, Pageable pageable) {
		return (Page<GenTable>)genTableDao.findAll(genTable, pageable);
	}

	public List<GenTable> findAll() {
		return genTableDao.findAll(new GenTable());
	}
	...
	public boolean checkTableName(String tableName){
		...
		List<GenTable> list = genTableDao.findAll(genTable);
		return list.size() == 0;
	}
