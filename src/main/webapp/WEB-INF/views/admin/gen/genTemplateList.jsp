...
<body>
	...
	<form:form id="searchForm" modelAttribute="genTemplate" action="${ctxAdmin}/gen/genTemplate/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="genTemplate">
			<tr>
				<td><a href="${ctxAdmin}/gen/genTemplate/form?id=${genTemplate.id}">${genTemplate.name}</a></td>
				...
				<shiro:hasPermission name="gen:genTemplate:edit"><td>
    				<a href="${ctxAdmin}/gen/genTemplate/form?id=${genTemplate.id}">修改</a>
					<a href="${ctxAdmin}/gen/genTemplate/delete?id=${genTemplate.id}" onclick="return confirmx('确认要删除该代码模板吗？', this.href)">删除</a>
